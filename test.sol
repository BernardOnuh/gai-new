...
interface IBizflipBundleMarketplace {
    function validateItemSold(address, uint256, uint256) external;
}

interface IBizflipNFTFactory {
...
interface IBizflipPriceFeed {
    function wFTM() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract BizflipMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IBizflipAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IBizflipBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createInvest(
        ...
        IBizflipAuction auction = IBizflipAuction(addressRegistry.auction());
    ...
    function approveAndTransfer(
        ...
        // Transfer NFT to buyer
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721(_nftAddress).safeTransferFrom(_owner, _creator, _tokenId);
        } else {
            IERC1155(_nftAddress).safeTransferFrom(
                _owner,
                _creator,
    ...
    function registerRoyalty(
        ...
        require(_isBizflipNFT(_nftAddress), "invalid nft address");
    ...
    function _isBizflipNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.artion() == _nftAddress ||
            IBizflipNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IBizflipNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IBizflipNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IBizflipNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IBizflipPriceFeed priceFeed = IBizflipPriceFeed(
    ...
    /**
     @notice Update BizflipAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IBizflipAddressRegistry(_registry);
    }
    ...
    require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IBizflipTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
